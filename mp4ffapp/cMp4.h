...
public:
  ...
  int32_t get_total_tracks() { return total_tracks; }
...
private:
  ...
  //{{{  read utils
  int64_t position();
  int32_t set_position (const uint64_t position);
  ...
  bool atom_compare (uint8_t* ch1, char* ch2);
  uint8_t atom_name_to_type (uint8_t* fourChars);
  uint64_t atom_read_header (uint8_t* atom_type, uint8_t* header_size, int indent);
  //}}}
  //{{{  meta
