...
/*{{{*/
static int32_t mp4ff_read_data (mp4ff_t* f, int8_t* data, uint32_t size) {
...
/*}}}*/
/*{{{*/
static int32_t mp4ff_write_data (mp4ff_t* f, int8_t* data, uint32_t size) {
...
static int32_t mp4ff_atom_get_size (const int8_t* data) {
  ...
  uint32_t result = ((uint8_t)data[0]) << 24;
  result |= ((uint8_t)data[1]) << 16;
  result |= ((uint8_t)data[2]) << 8;
  result |= (uint8_t)data[3];

  return (int32_t)result;
  }
/*}}}*/
/*{{{*/
static bool mp4ff_atom_compare (int8_t* ch1, int8_t* ch2) {
...
/*}}}*/
/*{{{*/
static uint8_t mp4ff_atom_name_to_type (int8_t* fourChars) {
...
/*}}}*/
/*{{{*/
static uint64_t mp4ff_atom_read_header (mp4ff_t* f, uint8_t* atom_type, uint8_t* header_size)  {
...
/*}}}*/
/*{{{*/
static int32_t mp4ff_parse_metadata (mp4ff_t* f, int32_t size) {
...
/*}}}*/
/*{{{*/
static int32_t mp4ff_atom_read (mp4ff_t* f, int32_t size, uint8_t atom_type) {
...
/*{{{*/
static int32_t parse_sub_atoms (mp4ff_t* f, const uint64_t total_size, int indent) {
// parse atoms that are sub atoms of other atoms
  ...
  while (counted_size < total_size) {
    ...
    // new track, update index, so that all data and tables get written in the right place
    if (atom_type == ATOM_TRAK)
      mp4ff_track_add (f);

    /* parse subatoms */
    if (atom_type < SUBATOMIC)
      parse_sub_atoms (f, size - header_size, indent + 1);
    else
